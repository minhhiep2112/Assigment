...
public class TC_API_Login_nangcao {
	...
	public void init() {
		...
		JSONObject jsonObject = (JSONObject) jsonParser.parse(new FileReader("./configuration/configs.json"));
		
		username =  jsonObject.get("account").toString();
		password = jsonObject.get("password").toString();
		baseURL = jsonObject.get("baseURL").toString();
		loginpath = jsonObject.get("loginPath").toString();
		...
		System.out.println(login_response.asPrettyString());
			...
			jsonObject.put("token", login_response.jsonPath().getString("token"));
			
			 try {
		         FileWriter file = new FileWriter("./configuration/configs.json");
